...
- `--benchmark-generation` is a separate quiet-host evidence lane. It requires `--max-new-tokens` of at least two and a baseline that actually generates at least two IDs before EOS, rejects `--timings` and `--trace-output`, performs 10 warm-ups plus 30 measured direct prefill/cached-decode iterations with device synchronization, requires positive finite durations, exact generated-ID replay, and at most 5% relative median absolute deviation, and writes one `candle-lfm2-vl-generation-benchmark-v1` JSON record to stderr without changing inference JSON.
