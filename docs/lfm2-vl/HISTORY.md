...
- Added the digest-pinned `synthetic-gpt-oss-task2-oracle-v1` fixture: 2,690
  ...
  generated by the standalone Python 3.13.3 / NumPy 2.3.3 scalar reference.
  It covers packed expert output, router selection, attention transitions,
  uncached logits, and cached prefill/decode logits at absolute tolerance
  `1e-4`.
- Added opt-in `GptOssResourceLimits` and exact logical KV accounting,
  `GptOssCancellationToken` prefill/decode rollback, logical/capacity usage
  reporting, and `GptOssLoadRegistry`/GGUF RAII ownership with duplicate
  rejection, cancellation cleanup, failed-load retry, and completed-load
  release.
- Focused native verification passed 27/27 GPT-OSS tests, including exact
  four-token/512-byte and one-over admission cases, cancellation/reset buffer
  release, and zero active/loaded ownership after load cleanup. The exact
  product GGUF, tokenizer, parity, and CUDA remain unclaimed.
