...
| Deterministic inference evidence | Bounded prompt/image/generation inputs, exact consumed-file hashes, native and hybrid prefill/decode, full-logit hashes, greedy top-k/token trace, and exact reset replay | Green; 33/33 focused example tests, including real split-MMProj hybrid execution, direct-Q8 hybrid evidence, exact direct/split/override source lists, one-line JSON, and native trace no-clobber publication |
...
| Native 3B production checkpoint | Pinned 3B config/tokenizer/processor, all 27 vision layers, projector/merge, prefill/decode, generated IDs, cache reset, consumed-file hashes, and cleanup | Gated; lock/config-only validation and bounded trace wiring are green, but the external 6,264,993,989-byte snapshot, pinned oracle, and production receipt are absent. No 3B compatibility claim is made. |
| Official 400M Q8_0 MMProj production execution | Identical-input dense/dequantized versus direct native-Q8 GGUF image features, prefill/decode, generated IDs, cache reset, retained Q8 count, hashes, and cleanup | Gated; deterministic hybrid fixture proves the separate evidence schema, Q8 retention, exact reset, projector cosine `>=0.9999`, and logits max abs `2e-2`; the official paired GGUF artifacts and production receipt are absent. |
...
---
AI-edited: 2026-08-21T12:40:00-04:00 | agent=Codex/root | model=gpt-5.6-sol | effort=ultra | task=lfm2-3b-q8-proof-gap | change=separated gated 3B and official Q8 production rows from fixture evidence
